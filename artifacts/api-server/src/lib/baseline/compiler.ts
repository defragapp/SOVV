...
function logWarn(event: string, detail?: unknown) { console.warn(`[baseline-compiler] ${event}`, detail ?? ""); }
...
type Astronomy = NonNullable<BaselineDesignDataset["astronomy"]>;
type AstroBodies = Astronomy["bodies"];
type Frameworks = NonNullable<BaselineDesignDataset["frameworks"]>;
...
async function geocodeLocation(pob: string): Promise<{
  ...
  try {
    ...
    try {
      ...
      logWarn("baseline_timezone_lookup_failed", error)
    }

    return { lat: latNum, lng: lngNum, timezone, displayName: display_name }
  } catch (error) {
    logWarn("baseline_geocode_failed", error)
    return null
  }
}
...
/**
 * Fetch every planet's position SEQUENTIALLY with a small gap. JPL Horizons hard
 * rate-limits concurrent requests — firing all bodies in parallel makes ~9/11
 * come back as 503 HTML error pages. Sequential + a brief gap resolves all bodies.
 */
async function fetchAllHorizonsPositions(
  datetime: string,
  lat: number,
  lng: number,
): Promise<Record<string, { longitude: number; latitude: number; retrograde: boolean }>> {
  const out: Record<string, { longitude: number; latitude: number; retrograde: boolean }> = {}
...
/**
 * Horizons returns ZERO ephemeris rows when START_TIME === STOP_TIME. Give it a
 * 1-day window (STOP = START + 24h) with STEP_SIZE=1d so it emits exactly one row,
 * at START. Input/output naive format: "YYYY-Mon-DD HH:MM".
 */
function horizonsStopPlusOneDay(datetime: string): string {
  const m = datetime.match(/^(\d{4})-([A-Za-z]{3})-(\d{2})\s+(\d{2}):(\d{2})$/)
  if (!m) return datetime
  const [, y, mon, d, hh, mm] = m
  const monIdx = HZ_MONTHS.indexOf(mon ?? "Jan")
  const base = new Date(Date.UTC(Number(y), monIdx < 0 ? 0 : monIdx, Number(d), Number(hh), Number(mm)))
  const next = new Date(base.getTime() + 24 * 60 * 60 * 1000)
  const nd = String(next.getUTCDate()).padStart(2, "0")
  const nhh = String(next.getUTCHours()).padStart(2, "0")
  const nmm = String(next.getUTCMinutes()).padStart(2, "0")
  return `${next.getUTCFullYear()}-${HZ_MONTHS[next.getUTCMonth()]}-${nd} ${nhh}:${nmm}`
}
...
async function fetchHorizonsPosition(
  ...
  try {
    ...
    const params = new URLSearchParams({
      ...
      COORD_TYPE: "GEODETIC",
      SITE_COORD: `'${lng.toFixed(4)},${lat.toFixed(4)},0'`,  // must be quoted or Horizons errors "Too many constants"
      START_TIME: `'${datetime}'`,
      STOP_TIME: `'${horizonsStopPlusOneDay(datetime)}'`,  // 1-day window → exactly one row at START
      STEP_SIZE: "1d",
    ...
    // Horizons returns 503 under concurrent load — back off and retry.
    if (res.status === 503 && attempt < 3) {
      await sleep(400 * (attempt + 1))
      return fetchHorizonsPosition(targetId, datetime, lat, lng, attempt + 1)
    }
    if (!res.ok) return null
    ...
    logWarn("baseline_planet_parse_failed", error)
    return null
  }
}
...
async function computeAstronomySnapshot(
  ...
  try {
    ...
    const positions = await fetchAllHorizonsPositions(datetime, lat, lng)
    for (const [name, pos] of Object.entries(positions)) {
      const { sign, degree } = eclipticLongitudeToSign(pos.longitude)
      bodies[name] = { longitude: pos.longitude, latitude: pos.latitude, sign, degree, retrograde: pos.retrograde }
    }
    ...
    logWarn("baseline_astronomy_snapshot_failed", error)
    return null
  }
}
...
const skyCache = new Map<string, { snapshot: LiveSkySnapshot; expiresAt: number }>();
...
export async function getCurrentSkySnapshot(
  ...
  const entry = skyCache.get(cacheKey)
  ...
  try {
    ...
    // Cache for 6 hours
    skyCache.set(cacheKey, { snapshot, expiresAt: Date.now() + 6 * 60 * 60 * 1000 })
...
function computeGeneKeysFramework(
  hdFramework: Frameworks["humanDesign"]
): Frameworks["geneKeys"] {
...
function computeNumerology(dob: string): Frameworks["numerology"] {
  const [year, month, day] = dob.split("-").map(Number)
...
async function synthesizeAIDataset(
  ...
  client: OpenAI,
  model: string
): Promise<NonNullable<BaselineDesignDataset["aiDataset"]> | null> {
  try {
    ...
    const completion = await client.chat.completions.create({ model, temperature: 0.2, max_tokens: 1200, response_format: { type: "json_object" }, messages: messages as any });
    const rawText = completion.choices[0]?.message?.content ?? "";
    const match = rawText.trim().match(/\{[\s\S]*\}/)
    if (!match) return null

    return JSON.parse(match[0]) as NonNullable<BaselineDesignDataset["aiDataset"]>
  } catch (error) {
    logWarn("baseline_ai_dataset_parse_failed", error)
    return null
  }
}
...
export async function compileBaselineDataset(
  ...
  try {
    ...
    const aiDataset = await synthesizeAIDataset(frameworks, client, model)
    if (aiDataset) {
