      ...
      <Section className="w-full relative flex flex-col items-center justify-center min-h-[50svh] pt-32 pb-24 overflow-hidden border-b border-border">
        <Container className="relative z-10 flex flex-col items-center text-center max-w-4xl">
          <div className="mb-10 flex items-center gap-3">
            ...
            <Badge variant="outline" className="rounded-none border-border bg-transparent text-[#71717A] font-sans font-medium text-[10px] tracking-[0.2em] uppercase px-3 py-1">
              PRICING
            </Badge>
            <div className="h-px w-8 bg-white/20" />
          </div>
      ...
      <Section className="w-full border-b border-border">
        <Container>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-white/[0.05]">
            {/* Free Plan */}
            <div className="bg-surface p-10 lg:p-14 flex flex-col gap-8">
              <div className="flex flex-col gap-2">
                ...
                <div className="flex items-baseline gap-1">
                  ...
                  <span className="text-[#71717A] font-sans font-medium text-[10px] tracking-wide uppercase">/{PRICING_CONFIG.free.period}</span>
                </div>
              ...
              <div className="flex-1">
                <ul className="space-y-0 border border-border divide-y divide-border">
                  {PRICING_CONFIG.free.features.map((feature, i) => (
                    <li key={i} className="flex items-start gap-4 px-4 py-3 font-sans font-medium text-xs text-[#71717A]">
              ...
              <div className="pt-6">
                <Link href={PRICING_CONFIG.free.href} className="w-full block">
                  <Button variant="secondary" className="w-full rounded-none border border-border bg-transparent text-[#A1A1AA] hover:text-white hover:border-border font-sans font-medium text-xs tracking-[0.15em] uppercase h-12 px-8 transition-colors">
            ...
            <div className="bg-surface p-10 lg:p-14 flex flex-col gap-8 border-l border-border relative">
              {PRICING_CONFIG.pro.highlight && <div className="absolute top-0 right-10 -translate-y-1/2">
                 <Badge className="rounded-none border border-border bg-white text-black font-sans font-medium text-[10px] tracking-[0.2em] uppercase px-3 py-1">{PRICING_CONFIG.pro.highlight}</Badge>
              </div>}
              <div className="flex flex-col gap-2">
                ...
                <div className="flex items-baseline gap-1">
                  ...
                  <span className="text-[#71717A] font-sans font-medium text-[10px] tracking-wide uppercase">/{PRICING_CONFIG.pro.period}</span>
                </div>
              ...
              <div className="flex-1">
                <ul className="space-y-0 border border-border divide-y divide-border">
                  {PRICING_CONFIG.pro.features.map((feature, i) => (
                    <li key={i} className="flex items-start gap-4 px-4 py-3 font-sans font-medium text-xs text-[#FAFAFA]">
