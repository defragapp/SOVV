...
export default function DefragItemPage() {
 ...
 <h3 className="text-[10px] text-[#4f4b47] uppercase tracking-[0.2em]">Library</h3>
 </div>
 <div className="flex-1 px-5 py-6">
 <p className="text-xs text-[#4f4b47] leading-relaxed">You are viewing a saved session. Provide updates to shift the pattern.</p>
 </div>
 </div>
 )
 ...
 <p className="text-[10px] text-[#4f4b47] uppercase tracking-[0.15em]">Baseline Design</p>
 ...
 <p className="text-[10px] text-[#4f4b47] uppercase tracking-[0.15em]">Audio Overview</p>
 {audioUrl ? (
 ...
 variant="secondary"
 className="rounded-none border border-border bg-transparent text-[#f4efe9] hover:bg-white/5 text-[10px] tracking-[0.15em] uppercase h-8 w-full"
 >
 {isGeneratingAudio ?"Generating..." :"Generate Audio"}
 </Button>
 {audioError && <p className="text-red-400 text-[10px] leading-tight">{audioError}</p>}
 {!audioError && !isGeneratingAudio && <p className="text-[10px] text-[#52525B] leading-tight">Requires Pro</p>}
 </div>
 )}
 </div>

 <div className="border border-border bg-surface p-4 flex flex-col gap-1.5">
 <p className="text-[10px] text-[#4f4b47] uppercase tracking-[0.15em]">Watch Preview</p>
 ...
 <h4 className="text-[10px] text-[#76716b] uppercase tracking-[0.15em] mb-3">{title}</h4>
 {isArray ? (
 <ul className="list-disc pl-4 space-y-2">
 {Array.isArray(content) ? content.map((item: string, i: number) => (
 <li key={i} className="text-sm text-[#f4efe9] leading-relaxed">{item}</li>
 )) : <li className="text-sm text-[#f4efe9] leading-relaxed">{String(content)}</li>}
 </ul>
 ) : (
 <p className="text-sm text-[#f4efe9] leading-relaxed whitespace-pre-wrap">{String(content)}</p>
 )}
 </div>
 )
 }
 ...
 <div className="flex-1 flex items-center justify-center border border-border bg-surface p-6 text-center">
 <p className="text-sm text-red-400 leading-relaxed max-w-sm">
 ...
 <h4 className="text-[10px] text-[#76716b] uppercase tracking-[0.15em] mb-3">Best Next Response</h4>
 <p className="text-sm text-[#f4efe9] leading-relaxed mb-4">{result.bestNextResponse.summary || String(result.bestNextResponse)}</p>
 ...
 <div key={i} className="text-sm text-[#E4E4E7] leading-relaxed flex items-start gap-3">
 ...
 <div>
 <h4 className="text-[10px] text-[#76716b] uppercase tracking-[0.15em] mb-3">Steer Toward</h4>
 <ul className="space-y-2">
 {Array.isArray(result.conversationalSteering.do) ? result.conversationalSteering.do.map((item: string, i: number) => (
 <li key={i} className="text-sm text-[#E4E4E7] flex items-start gap-2">
 <span className="text-[#10B981]">+</span>
 <span>{item}</span>
 </li>
 )) : <li className="text-sm text-[#E4E4E7]">{String(result.conversationalSteering)}</li>}
 </ul>
 </div>
 <div>
 <h4 className="text-[10px] text-[#76716b] uppercase tracking-[0.15em] mb-3">Avoid</h4>
 <ul className="space-y-2">
 ...
 <div className="bg-red-500/10 border border-red-500/20 text-red-400 text-sm p-4 mb-4">
 {error}
 </div>
 )}
 ...
 className="w-full bg-transparent text-[#f4efe9] placeholder:text-[#4f4b47] resize-none outline-none min-h-[120px] text-sm p-4 leading-relaxed"
 onKeyDown={(e) => {
 ...
 <span className="text-[10px] text-[#4f4b47] tracking-wide">ENTER TO UPDATE</span>
 <Button
 size="sm"
 onClick={handleUpdate}
 disabled={!input.trim() || isLoading}
 className="rounded-none border border-border bg-white text-black hover:bg-white/90 text-[10px] tracking-[0.15em] uppercase h-8 px-4 disabled:opacity-20 disabled:cursor-not-allowed transition-colors"
 >
