        ...
        {result && (
          <div className="flex flex-col gap-6 mt-4">
            ...
            <div className="border border-border bg-surface p-5 flex flex-col gap-3">
              <p className="text-[10px] font-sans font-medium text-[#71717A] uppercase tracking-[0.15em]">Audio Overview</p>
              {audioUrl ? (
                ...
                <div className="flex flex-col gap-3 mt-1">
                  ...
                  {!audioError && !isGeneratingAudio && <p className="text-[10px] text-[#71717A] font-sans font-medium leading-tight">Requires Pro</p>}
                </div>
              )}
            </div>

            <div className="border border-border bg-surface p-5 flex flex-col gap-2 opacity-40">
              <p className="text-[10px] font-sans font-medium text-[#71717A] uppercase tracking-[0.15em]">Watch Preview</p>
    ...
    <div className="flex flex-col h-full justify-end gap-8 pt-4 pb-0 max-w-4xl mx-auto w-full">
      ...
      <div className="border border-border bg-surface focus-within:border-border transition-colors duration-200 shadow-2xl">
        ...
        <div className="flex justify-between items-center px-5 py-4 border-t border-border bg-surface">
          <span className="text-[10px] text-[#71717A] font-sans font-medium tracking-[0.15em] uppercase">ENTER TO DEFRAG</span>
          <Button
            size="sm"
        ...
        {isArray ? (
          <ul className="list-none space-y-3">
            {Array.isArray(content) ? content.map((item: string, i: number) => (
              <li key={i} className="text-[14px] text-[#FAFAFA] font-sans font-medium leading-[1.75] flex items-start gap-3">
                 <span className="text-[#3F3F46] mt-0.5 shrink-0">→</span>
                 <span>{item}</span>
              </li>
            )) : <li className="text-[14px] text-[#FAFAFA] font-sans font-medium leading-[1.75]">{String(content)}</li>}
          </ul>
        ) : (
        ...
        <div className="flex-1 overflow-y-auto border border-border bg-surface p-8 md:p-12 shadow-xl">
           ...
           {result.bestNextResponse && (
             <div className="border-b border-border pb-8 mb-8">
                <h4 className="text-[10px] font-sans font-medium text-[#71717A] uppercase tracking-[0.2em] mb-4">Best Next Response</h4>
                <p className="text-[14px] text-[#FAFAFA] font-sans font-medium leading-[1.75] mb-5">{result.bestNextResponse.summary || String(result.bestNextResponse)}</p>
                {Array.isArray(result.bestNextResponse.phrasing) && result.bestNextResponse.phrasing.length > 0 && (
                   <div className="bg-surface border border-border p-6 flex flex-col gap-3">
                      {result.bestNextResponse.phrasing.map((phrase: string, i: number) => (
                        <div key={i} className="text-[14px] text-[#A1A1AA] font-sans font-medium leading-[1.75] flex items-start gap-3">
                           <span className="text-[#3F3F46] mt-0.5 shrink-0">↳</span>
                           <span>{phrase}</span>
                        </div>
                      ))}
           ...
           {result.conversationalSteering && (
             <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                  <h4 className="text-[10px] font-sans font-medium text-[#71717A] uppercase tracking-[0.2em] mb-4">Steer Toward</h4>
                  <ul className="space-y-3">
                    {Array.isArray(result.conversationalSteering.do) ? result.conversationalSteering.do.map((item: string, i: number) => (
                      <li key={i} className="text-[14px] text-[#A1A1AA] font-sans font-medium leading-[1.75] flex items-start gap-3">
                        <span className="text-[#10B981] mt-0.5 shrink-0">+</span>
                        <span>{item}</span>
                      </li>
                    )) : <li className="text-[14px] text-[#A1A1AA] font-sans font-medium leading-[1.75]">{String(result.conversationalSteering)}</li>}
                  </ul>
                </div>
                <div>
                  <h4 className="text-[10px] font-sans font-medium text-[#71717A] uppercase tracking-[0.2em] mb-4">Avoid</h4>
                  <ul className="space-y-3">
