...
export async function extractPatterns(env, sessionId, newInteractionId) {
    try {
        ...
        if (interactions.length < 2)
            return; // Need at least 2 interactions to find patterns
  ...
  Confidence: ${i.confidence}`;
        ...
        for (const pattern of parsed.patterns) {
            ...
            if (pattern.matches_known) {
                await env.DB.prepare("UPDATE patterns SET occurrence_count = occurrence_count + 1, last_seen = ? WHERE session_id = ? AND content = ?").bind(Date.now(), sessionId, pattern.matches_known).run();
            }
            else {
...
export async function handlePatternVerify(req, env) {
    ...
    await env.DB.prepare("UPDATE patterns SET verified = ?, last_seen = ? WHERE id = ? AND session_id = ?").bind(verified, Date.now(), body.patternId, sid).run();
    return Response.json({ success: true, patternId: body.patternId, verified }, { headers: { "set-cookie": cookieHeader(sid), "cache-control": "no-store" } });
}
export async function handleGetPatterns(req, env) {
    const user = await verifyAccessJWT(req);
    if (!user) {
        return new Response("Unauthorized", { status: 401 });
    }
    ...
    return Response.json({ patterns }, { headers: { "set-cookie": cookieHeader(sid), "cache-control": "no-store" } });
}
